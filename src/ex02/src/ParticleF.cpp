...
void ParticleF::StepFilter(Particle *currentParticles){
    ...
    float dAng = abs(newOdomPose.ang - prevOdomPose.ang);
    const float ANG_THRESH = M_PI/6;
...
void ParticleF::InitFilter(void){   
    ...
    laser_sub = node.subscribe<sensor_msgs::LaserScan>("base_scan", 1000, &ParticleF::LaserCallback, this);
    ...
    estPose_pub = node.advertise<geometry_msgs::PoseStamped>("pose_estimate", 1);
...
void ParticleF::PublishState(void){
    ...
    stamped.pose.position.x = predictedParticle.pose.x;
    stamped.pose.position.y = predictedParticle.pose.y;
    stamped.pose.position.z = 0;
    
    stamped.pose.orientation = QuaternionFromEuler(0,0,predictedParticle.pose.ang);
    
    estPose_pub.publish(stamped);
...
Particle ParticleF::EstimatePose( void ){
    ...
    while (numOfClusters < maxClusters){
        ...
        if (mostLikelyUnprocessedParticle != NULL){
            ...
            if (c->weightSum / weightSum > certaintyBreak){
                break;
            }
