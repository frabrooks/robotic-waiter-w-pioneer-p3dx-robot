...
// mark empty space, the calculated range should be greater than max range in this case
        // f != f if nan
        si.noHit = si.distance != si.distance;
        
        // count number of valid scans vs expected
        if(!si.noHit){
            hits++;
        }
        laserReadings[i] = si;
    ...
    laser_reliable_raw = hits/expectedHits;

    laser_trust = ClampLerp(laser_reliable_raw,0.65,0.85);
...
Particle ParticleF::SensorUpdate(SensorInfo *info, Particle particle) {
    ...
    comb_reliable_raw = 1-(misses/LASER_SAMPLE);
    
#ifdef ABSERROR

    particle.weight = 0.3 * particle.weight;
    //printf("%f|",wx);
    float addWeight = (1.0f - (error / maxError));
    particle.weight += addWeight;
...
double ParticleF::map_calc_range(Pose origin, nav_msgs::OccupancyGrid* occupancyGrid) {
    ...
    double max_range = laser_rngMax/map_resolution;
    ...
    return max_range * map_resolution;
}
